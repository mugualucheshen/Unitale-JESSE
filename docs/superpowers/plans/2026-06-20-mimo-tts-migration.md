...
**Expected:** summary 显示 "通过 19 / 失败 0"(3+2+2+3+4+5 = 19:Task3 三个用例 + Task4 两个 + Task5 两个 + Task6 三个 + Task7 四个 + Task8 五个)
...
(function migrateTtsConfig() {
  ...
  if (localStorage.getItem('storyforge_tts_schema_version') !== SCHEMA_VERSION) {
    try {
  ...
  try {
    ...
    if (ttsRefPath.value) {
      ...
      if (timbre && !timbre.source) {
        ...
        // 写回 IndexedDB(懒补)。失败仅警告,不阻断主流程
        try { saveAssetToDB(timbre); }
        catch(e) { console.warn(`[TTS] timbre.source 补全失败 refPath=${timbre.refPath}`, e); }
      }
    }
...
- [ ] **Step 2:辅助函数存在性检查(必须,缺失则阻塞)**

```bash
grep -n "function arrayBufferToBase64\|getAssetFromDB\|saveAssetToDB" index.html
```

**必须**看到:
- `arrayBufferToBase64` 函数定义(若缺失,补充如下;若已存在则跳过)
...
- `getAssetFromDB(refPath)` 函数定义
- `saveAssetToDB(fileOrObj, ...)` 函数定义

**任一缺失 → 本 task 阻塞**,先补齐再继续。
...
```bash
grep -n "generateQwenVoice\|useCustomQwenVoiceText\|customQwenVoiceTextTemplate\|defaultQwenVoiceTextTemplate\|qwenUrl\|qwen/design\|Qwen3TTS\|storyforge_use_custom_qwen_voice_text\|storyforge_qwen_voice_text_template\|qwen_\${char.name}\|/v1/qwen/design" index.html
```
...
<button @click="testTtsConnection(conf)" type="button"
  ...
  <span v-if="ttsTestResults[conf.id]?.status === 'ok'" class="text-green-600">✓ {{ ttsTestResults[conf.id].voice }}</span>
  <span v-if="ttsTestResults[conf.id]?.status === 'fail'" class="text-red-500">✗</span>
</button>
```
...
// ttsTestResults: { [confId]: 'ok' | 'fail' | { voice: string } }
const ttsTestResults = ref({});

const testTtsConnection = async (conf) => {
  if (!conf.apiKey) {
    ttsTestResults.value = {...ttsTestResults.value, [conf.id]: 'fail'};
    return alert('请先填写 apiKey');
  }
  testingTtsId.value = conf.id;
  ttsTestResults.value = {...ttsTestResults.value, [conf.id]: null};
  const controller = new AbortController();
  try {
    const payload = MiMoTTS.buildSynthesizePayload('测试', conf.defaultVoice, '');
    const blob = await MiMoTTS.synthesize(conf.apiKey, payload, controller.signal);
    if (blob.size > 0) {
      ttsTestResults.value = {...ttsTestResults.value, [conf.id]: {status: 'ok', voice: conf.defaultVoice}};
    } else {
      ttsTestResults.value = {...ttsTestResults.value, [conf.id]: {status: 'fail'}};
    }
  } catch (e) {
    if (e.name === 'AbortError') return;  // 用户主动取消不报错
    ttsTestResults.value = {...ttsTestResults.value, [conf.id]: {status: 'fail'}};
...
**注意:**
- 列表项展示用 `ttsTestResults[conf.id].status` 与 `ttsTestResults[conf.id].voice`,实现 per-config 显示
- `AbortController` 留作后续若加"停止测试"按钮时使用

- [ ] **Step 4:return 对象加测试状态**

```js
testingTtsId, ttsTestResults, testTtsConnection,
```

(`lastTtsTestVoice` 已废弃,改为 per-config `ttsTestResults[conf.id].voice`)
...
- [ ] **Step 2:加 showMigrationNotice ref 与 dismiss 函数(同步初始化,避免响应式时序问题)**

```js
// 在 Vue setup 顶部、数据迁移 IIFE 之后定义。同步读取 localStorage,避免 ref 创建后再赋值的响应式时序问题
const showMigrationNotice = ref(
  localStorage.getItem('storyforge_tts_migration_notice_dismissed_v1') === null
  && localStorage.getItem('storyforge_tts_schema_version') === '1'
);
...
2. 合成
3. **Expected:** 浏览器弹 alert "MiMo 鉴权失败,请检查 apiKey"(`synthesizeAudio` 走 ttsError 分支,无 UI badge 变化;alert 弹窗是主要反馈)
